 ...
 *
 * Covers:
 * 1. Sidebar labels match employer vocabulary (درخواست ها، بازار محصولات، مشاور AI …)
...
test.describe('ناوبری — alias route‌ها', () => {
  ...
  test('/marketplace باید به /catalog ریدایرکت شود', async () => {
    ...
    const currentPath = new URL(p.url()).pathname;
    expect(/\/(catalog|marketplace|downloads)/.test(currentPath)).toBe(true);
    await expect(p.locator('main').first()).toBeVisible({ timeout: 8000 });
  });
});
...
test.describe('صفحه جزئیات درخواست', () => {
  test.describe.configure({ mode: 'serial' });
...
test.describe('چک‌لیست دمو — سناریوی اصلی', () => {
  ...
  test('۳. ناوبری کامل — همه آیتم‌های اصلی sidebar دیده می‌شوند', async () => {
    const mainNavItems = ['داشبورد', 'محصولات', 'درخواست ها و فرصت ها', 'مالی'];
    for (const item of mainNavItems) {
      await expect(p.getByRole('link', { name: item }).first()).toBeVisible({ timeout: 5000 });
    }
  });

  test('۴. صفحه درخواست ها — دسترسی و نمایش لیست', async () => {
    await p.click('a[href="/rfq"]');
    await expect(p).toHaveURL(/\/trade/, { timeout: 8000 });
